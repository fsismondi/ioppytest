from coap_testing_tool import TD_DIR, TD_COAP, TD_COAP_CFG, TD_6LOWPAN_FORMAT
...
class ImportYamlInteropTestCases(unittest.TestCase):
    ...
    def test_yaml_testcase_syntax_6lowpan(self):
        imported_tcs = import_teds(TD_6LOWPAN_FORMAT)
        for tc in imported_tcs:
            print(tc)
            assert tc.id
            assert tc.uri
    ...
    # def test_yaml_testcase_configuration_syntax_6lowpan(self):
    #     imported_configs = import_teds(TD_6LOWPAN_CFG)
    #     for tc_config in imported_configs:
    #         print(tc_config)
    #         assert tc_config.id
    #         assert tc_config.uri
